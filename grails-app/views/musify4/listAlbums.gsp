...
<head>
    ...
    <link rel="stylesheet" href="${resource(dir: 'css', file: 'musify4.css')}" type="text/css">
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#searchButton").click(function(event) {
                ...
                let url = "${createLink(controller:'Musify4', action:'search')}";
                $.ajax({url: url, data: formData, dataType: "json", success: function(result) {
                        ...
                        for(album of result)
                        {
                            ...
                            row.insertCell(3).outerHTML = "<th id=editCell><a href=/Musify4/musify4/edit/"+parseInt(album.id)+" class='btn btn-primary' value="+album.id+">Edit</a></th>";
                            row.insertCell(4).outerHTML = "<th id=deleteCell><a href=/Musify4/musify4/delete/"+parseInt(album.id)+" class='btn btn-danger' value="+album.id+">Delete</a></th>";
                        }
                    }});
               ...
               event.preventDefault();
                ...
                $.ajax({url: url, dataType: "json", success: function(result) {
                        ...
                        for(album of result)
                        {
                            ...
                            row.insertCell(1).outerHTML = "<th id=titleCell><label>"+album.title+"</label></th>";
                            row.insertCell(2).outerHTML = "<th id=genresCell><label id=\"genres\">"+properGenres+"</label></th>";
                            row.insertCell(3).outerHTML = "<th id=editCell><a href=/musify4/edit/"+parseInt(album.id)+" class='btn btn-primary' value="+album.id+">Edit</a></th>";
                            row.insertCell(4).outerHTML = "<th id=deleteCell><a href=/musify4/delete/"+parseInt(album.id)+" class='btn btn-danger' value="+album.id+">Delete</a></th>";
                        }
                    }});
                $('#titleField').val("");
                $('#artistField').val("");
...
<body>
    ...
    <div id="formDiv">
        <form id="myForm" name="myForm" controller="Musify4">
